...
static void init(struct fmt_main *self)
{
	...
	maxsize = get_kernel_max_lws(ocl_gpu_id, GenerateSHA512pwhash);
	maxsize2 = get_kernel_max_lws(ocl_gpu_id, crypt_kernel);
	if (maxsize2 < maxsize) maxsize = maxsize2;
	maxsize2 = get_kernel_max_lws(ocl_gpu_id, Generate2013key);
	if (maxsize2 < maxsize) maxsize = maxsize2;
...
static int crypt_all(int *pcount, struct db_salt *salt)
{
	...
	HANDLE_CLERROR(clEnqueueNDRangeKernel(queue[ocl_gpu_id], Generate2013key, 1, NULL, &gws, &local_work_size, 0, NULL, lastEvent), "failed in clEnqueueNDRangeKernel");
